...
const DetailsPage: React.FC<RouteComponentProps<any>> = (props) => {
  ...
  const openModal = ({ urlPhoto }: { urlPhoto: string }) => {
    ...
    // setNewUrl(urlPhoto)
    setNewUrl(imgFilt[0]?.altImg1);
  };
  const openModal2 = ({ urlPhoto2 }: { urlPhoto2: string }) => {
    setShowModal(true);

    // setNewUrl(urlPhoto2)
    setNewUrl(imgFilt[0]?.altImg2);
  };
  const openModal3 = ({ urlPhoto3 }: { urlPhoto3: string }) => {
    setShowModal(true);
        ...
        <Container className={classes.contWidth}>
          <Grid container direction="column" alignItems="center">
            <Grid
              ...
              justify="center"
              className={classes.typographyContMargin}
            >
              <Typography variant="h3" className={classes.typoMargin}>
                {imgFilt[0]?.title}
              </Typography>
              <Typography variant="body1">
                Description over the item and it's images.
              </Typography>
            </Grid>
            ...
            <Grid container item spacing={3}>
              ...
              <Grid
                ...
                sm={5}
                md={2}
                onClick={() => openModal2({ urlPhoto2 })}
              >
                <Card className={classes.card} elevation={1}>
                  <CardActionArea className={classes.cardActionHeight}>
                    <CardMedia
                      className={classes.cardMedia}
                      image={imgFilt[0]?.altImg2}
                      title={imgFilt[0]?.title}
                    />
                  </CardActionArea>
                </Card>
